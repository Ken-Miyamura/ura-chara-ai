...
- SNS投稿のトーンと言語パターン
- SNSをやっていない場合、その理由自体を性格シグナルとして分析（例：「面倒」→行動力、「見られたくない」→社交性、「無駄」→論理性）
...
パブリックな性格を構築する。
ベースにするもの：データが明示的に示すもの、他者が報告する第一印象、SNSで見せるペルソナ（SNSをやっていない場合は、趣味・第一印象・スケジュールから推定）。
...
投稿の微妙な言語的手がかり。
SNSをやっていない場合：「やらない理由」の裏にある本音（例：本当は見たいけど比較が辛い、実は発信欲がないわけではない等）を深読みする。
...
- If the user doesn't use SNS, analyze their reason itself as a personality signal (e.g., "too lazy" → drive, "don't want to be seen" → sociability, "waste of time" → logic)
...
Base it on: what the data explicitly shows, first impressions others report, the persona shown on SNS (if the user doesn't use SNS, infer from hobbies, first impressions, and schedule instead).
...
If the user doesn't use SNS: read between the lines of their reason (e.g., they say they don't care but actually crave connection, or they claim it's a waste of time but secretly browse).
...
- Si el usuario no usa redes sociales, analiza la razón en sí como señal de personalidad (ej: "da pereza" → iniciativa, "no quiero que me vean" → sociabilidad, "es una pérdida de tiempo" → lógica)
...
Básate en: lo que los datos muestran explícitamente, las primeras impresiones que otros reportan, la persona que muestra en redes sociales (si el usuario no usa redes, infiere de pasatiempos, primeras impresiones y rutina).
...
Si el usuario no usa redes: lee entre líneas de su razón (ej: dicen que no les importa pero en realidad anhelan conexión, o dicen que es una pérdida de tiempo pero navegan en secreto).
